...
//	TFitter *virtFit = new TFitter(nPar);\
	...
	nestedMinuit->DefineParameter(0, "1", 	0.0, 0.1, -5.0, 5.0);
	nestedMinuit->DefineParameter(1, "2", 	0.0, 0.1, -5.0, 5.0);
...
{	
	...
	while (fReader.Next())
	{
		...
		else if(*geo==2)
		{
			...
			SQLang = (50.0 + inPar[0]) * TMath::DegToRad();
			SQRang = (15.0 + inPar[1]) * TMath::DegToRad();
			tarThickness = 160.0;			
		...
		else if(*geo==3)
		{
			...
			SQLang = (35.0 + inPar[0]) * TMath::DegToRad();
			SQRang = (15.0 + inPar[1]) * TMath::DegToRad();
			tarThickness = 160.0;
...
*/		
		...
		double fMWPC_1_X = *MWPC_1_X + params[0];
		double fMWPC_1_Y = *MWPC_1_Y;
		double fMWPC_2_X = *MWPC_2_X + params[1];
	...
	*/
	...
	TString paramString4 = TString::Format("inPar[2]: %.3f, inPar[1] = %.3f",inPar[0], inPar[1]);
	TString paramString5 = TString::Format("tarPos1: %.3f, tarPos23: = %.3f",tarPosZ, tarPosZ+2);
	...
	TString pathname = TString::Format("/home/zalewski/Desktop/move/pic_%.3f_%.3f.png", params[4], inPar[0]);
...
/*
	...
	TString pathname = TString::Format("/home/zalewski/Desktop/pic_%.3f_%.3f.png", params[4], params[1]);
	myResPlotsCanv->Print(pathname.Data());
	*/
	}
