...
/*
	TF1 *evX_mml = new TF1("evX_mml","[0]*x + [1]");
	graph_evX_mml =  new TGraph(stdV_Sqlang_2H.size(), &stdV_Sqlang_2H[0], &stdV_Sqrang_2H[0]);
...
bool dE_E_angle::create_output_tree(TTree *out)
{
	...
	outTree->Branch("deu",	&angAng1H,	"deu/O");
	outTree->Branch("pro",	&angAng2H,	"pro/O");
...
Float_t dE_E_angle::actual_work(Int_t geoNo)
{
	...
	for (Long64_t entry=0; entry<nEntries; entry++)
		...
		if ((in_aF5>600 && in_aF5<1200 && in_tof>175 && in_tof<190) || (in_aF5==0 && in_tof>175 && in_tof<190))
		{
...
Float_t dE_E_angle::actual_work_gas(Float_t MWPC_X_1_shift, Float_t MWPC_X_2_shift, Double_t qualityControl[4])
{
	...
	for (Long64_t entry=0; entry<nEntries; entry++)
		...
		he6=false, angAng1H=false, angAng2H=false, leftTele = false, rightTele = false;
		...
		//displacement + go to corner of MWPC + follow wire order - get event point
		MWPC_1_X += rnd->Uniform(0.0,1.25)-0.6125 + MWPC_X_1_shift;
		MWPC_1_Y += rnd->Uniform(0.0,1.25)-0.6125;
		MWPC_1_Z = -816.0;
		MWPC_2_X += rnd->Uniform(0.0,1.25)-0.6125 + MWPC_X_2_shift;
		MWPC_2_Y += rnd->Uniform(0.0,1.25)-0.6125;
		MWPC_2_Z = -270.0;
		...
		//SQR energy - helium
		if(SQX_R_mult*SQY_R_mult==1 && leftTele)
		{
			rightTele = true;
			sqrde=SQX_R_Edep[1];
			sqretot=memE_CsI_R;
		...
		{			
			if (deut_geo5->IsInside(sqrang,sqlang) && (sqlde+sqletot)>5.0)
			{
				he6=true;
				vec_evX.push_back(evX);
				vec_mml.push_back(mml);
				outTree->Fill();
			}
		}
	...
	evX_vs_mml_graph =  new TGraph(vec_evX.size(), &vec_evX[0], &vec_mml[0]);
